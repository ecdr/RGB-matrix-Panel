...
// (Actually the compiler should catch this, but it doesn't seem to care if omit pwidth)
  ...
  for (i = 1; i < nBuf; i++)
    matrixbuff[i] = &matrixbuff[0][buffsize * i];

// FIXME: adapt to handle dbuf == false
...
uint16_t RGBmatrixPanel::getPixel(uint8_t buf, int16_t x, int16_t y) {
  ...
  if(y < nRows) {
    // Data for the upper half of the display is stored in the lower
    // bits of each byte.
    ptr = &matrixbuff[buf][y * WIDTH * nPackedPlanes + x]; // Base addr
// plane 0 trick
    // Plane 0 is a tricky case -- its data is spread about,
    // stored in least two bits not used by the other planes.
...
// end plane 0 trick
    ...
    // Data for the lower half of the display is stored in the upper
    // bits, except for the plane 0 stuff, using 2 least bits.
    ptr = &matrixbuff[buf][(y - nRows) * WIDTH * nPackedPlanes + x];
// plane 0 trick
...
// TODO: add buffer number argument (so can load arbitrary buffer)
